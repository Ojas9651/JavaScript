...
form.addEventListener('submit', (e)=>{
    ...
    console.log(fname, " ",lname, " ",day, " ", month, " ", year);
    ...
    div.innerHTML = `
        Name: ${fname}<br><br>
        SurName: ${lname}<br><br>
        DOB: ${day}-${month}-${year}<br><br>
        Zodiac: ${zodiacSigns[month-1]}<br><br>
        You are: ${compliments[day-1]}<br><br>
        Your problem: ${victimCardCompliments[year%20]}<br><br>
        We Recommend: ${recommendations[day-1]}<br><br>
        We hope: ${predictions[(fname.length+lname.length)%20]}<br><br>
    `
    form.reset();
})